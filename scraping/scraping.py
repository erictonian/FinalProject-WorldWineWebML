...
BASE_URL = 'https://www.winemag.com/?s=&drink_type=wine&page={0}'
...
class Scraper():
    ...
    def __init__(self, pages_to_scrape=(1, 1), num_jobs=1, clear_old_data=True):
        ...
        self.estimated_total_reviews = (
            pages_to_scrape[1] + 1 - pages_to_scrape[0]) * 30
    ...
    def scrape_site(self):
        ...
        if self.multiprocessing:
            link_list = [BASE_URL.format(page) for page in range(
                self.pages_to_scrape[0], self.pages_to_scrape[1] + 1)]
    ...
    def parse_review(self, review_soup):
        ...
        description = review_soup.find(
            "p", {"class": "description"}).contents[0]

        try:
            taster_name = review_soup.find(
                "div", {"class", "taster"}).find("div", {"class", "name"})
            if taster_name is not None:
                taster_name = taster_name.contents[0]

            taster_twitter_handle = review_soup.find(
                "div", {"class", "taster"}).find("div", {"class", "twitter-handle"})
        ...
        if review_format['designation_index'] is not None:
            try:
                designation = info_containers[review_format['designation_index']].find(
                    'div', {'class': 'info'}).span.span.contents[0]
            except:
        ...
        if review_format['appellation_index'] is not None:
            appellation_info = info_containers[review_format['appellation_index']].find(
                'div', {'class': 'info'}).span.findChildren()
            try:
    ...
    def determine_review_format(self, review_soup):
        ...
        try:
            review_format['designation_index'] = review_info.index(
                'designation')
        except ValueError:
        ...
        try:
            review_format['appellation_index'] = review_info.index(
                'appellation')
        except ValueError:
    ...
    def update_scrape_status(self):
        ...
        time_remaining = round((self.estimated_total_reviews - self.cross_process_review_count)
                               * (self.cross_process_review_count / elapsed_time), 2)
...
if __name__ == '__main__':
    ...
    winmag_scraper = Scraper(
        pages_to_scrape=pages_to_scrape, num_jobs=10, clear_old_data=False)
